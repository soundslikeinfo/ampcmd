...
_ampcmd_validate_chain() {
	...
	if ! bash -n -c "$chain" >/dev/null 2>&1; then
...
ampcmd() {
	...
	history | tail -n "$num_lines" | awk '{for(i=2;i<=NF;i++) printf "%s ", $i; print ""}' | sed 's/\\n/ /g; /^[[:space:]]*$/d' | awk '!seen[$0]++' | nl -w3 -s' │ ' > "$hist_tmp"
	[[ -f "$_CHAINHIST_HISTORY" ]] && cat "$_CHAINHIST_HISTORY" | sed 's/^[0-9-]* [0-9:]* │ //; s/\\n/ /g; /^[[:space:]]*$/d' | nl -w3 -s' │ ' > "$chains_tmp"
